...
const SBF_ENGINE_OFF: usize = 600;
...
fn encode_init_market_with_invert(
    ...
    // warmup_period_slots must be > 0 — percolator@cf35789 (PERC-8093) validates this
    encode_init_market_full_v2(admin, mint, feed_id, invert, 0, 100)
}
...
fn encode_init_market_hyperp(admin: &Pubkey, mint: &Pubkey, initial_mark_price_e6: u64) -> Vec<u8> {
    ...
    // warmup_period_slots must be > 0 — percolator@cf35789 (PERC-8093) validates this
    encode_init_market_full_v2(admin, mint, &[0u8; 32], 0, initial_mark_price_e6, 100)
}
...
fn encode_init_market_full(
    ...
    data.extend_from_slice(&0u64.to_le_bytes()); // initial_mark_price_e6 (0 for non-Hyperp)
                                                 // RiskParams
                                                 // warmup_period_slots must be > 0 — percolator@cf35789 (PERC-8093) validates this
    data.extend_from_slice(&100u64.to_le_bytes()); // warmup_period_slots
    data.extend_from_slice(&500u64.to_le_bytes()); // maintenance_margin_bps
    data.extend_from_slice(&1000u64.to_le_bytes()); // initial_margin_bps
    data.extend_from_slice(&0u64.to_le_bytes()); // trading_fee_bps
    data.extend_from_slice(&(MAX_ACCOUNTS as u64).to_le_bytes());
...
fn test_hyperp_init_market_with_valid_price() {
    ...
    let ix = Instruction {
        ...
        data: encode_init_market_full_v2(
            ...
            100,              // warmup (must be > 0 since PERC-8093)
        ),
    };
...
impl TestEnv {
    ...
    fn read_insurance_balance(&self) -> u128 {
        let slab_account = self.svm.get_account(&self.slab).unwrap();
        // ENGINE_OFF = SBF_ENGINE_OFF (600), InsuranceFund.balance is at offset 16 within engine
        // (vault is 16 bytes at 0, insurance_fund starts at 16)
        // InsuranceFund { balance: U128, ... } - balance is first field
        const INSURANCE_OFFSET: usize = SBF_ENGINE_OFF + 16;
        u128::from_le_bytes(
...
impl TradeCpiTestEnv {
    ...
    fn read_insurance_balance(&self) -> u128 {
        ...
        const INSURANCE_BALANCE_OFFSET: usize = SBF_ENGINE_OFF + 16;
        u128::from_le_bytes(
    ...
    fn read_pnl_pos_tot(&self) -> u128 {
        ...
        const PNL_POS_TOT_OFFSET: usize = SBF_ENGINE_OFF + 576;
        u128::from_le_bytes(
    ...
    fn read_c_tot(&self) -> u128 {
        ...
        const C_TOT_OFFSET: usize = SBF_ENGINE_OFF + 560;
        u128::from_le_bytes(
    ...
    fn read_vault(&self) -> u128 {
        ...
        const VAULT_OFFSET: usize = SBF_ENGINE_OFF;
        u128::from_le_bytes(
...
fn test_attack_withdraw_bypasses_fee_debt() {
    ...
    // warmup_period_slots must be > 0 per PERC-8093
    env.init_market_with_warmup(0, 100);
